sap.ui.define([
	...
	return BaseObject.extend("hcm.capr.ZHCMApprove.model.GroupSortState", {
		 ...
		 * @alias hcm.capr.ZHCMApprove.model.GroupSortState
		 */
		...
		/**
		 * Groups by Total, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Total") {
				...
				this._oViewModel.setProperty("/sortBy", "Total");

				aSorters.push(
					new Sorter("Total", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Claimno");
			}
