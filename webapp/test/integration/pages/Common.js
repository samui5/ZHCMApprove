sap.ui.define([
	...
	function getFrameUrl (sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("hcm/capr/ZHCMApprove/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#ClaimApproval-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#ClaimApproval-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("hcm.capr.ZHCMApprove.test.integration.pages.Common", {
		...
		getMockServer : function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["hcm/capr/ZHCMApprove/localService/mockserver"], function (mockserver) {
